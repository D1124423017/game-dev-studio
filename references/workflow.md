...
- Client Studio Production Workflow：甲方委託、完整製作、從企劃到實作與驗收。
...
當使用者是甲方、客戶、stakeholder 或 product owner，要求「幫我做一款遊戲」、「委託你做」、「從企劃到實作」時，使用 Client Studio Production Workflow。

1. 先整理 client brief，不要直接開始寫程式
2. 提出 2-4 個 studio proposal 方向
3. 鎖定 MVP、延期項目、不做項目與驗收標準
4. 定義正式遊戲美術方向、UI visual、VFX、motion、生圖方案與 visual target / style bible
5. 定義 production architecture 與模組邊界
6. 若是多階段製作，定義 Prototype、Vertical Slice、Alpha、Beta、Release Candidate 或驗收 gate
7. 拆成 Codex 可執行任務
8. 實作前讀現有 repo，避免破壞架構，並保留 scope、visual target、architecture 與 acceptance criteria
9. 完成後做 QA、犀利測試玩家回饋、implementation delivery report 與 client acceptance report

詳細規則見 `references/client-studio-production-workflow.md`。
...
3. 切換企劃角度：玩法、核心循環、爽點
4. 切換技術角度：引擎、平台、架構
5. 切換美術角度：風格、資產需求、visual target、style bible
...
1. 先確認影響範圍
2. 先讀現有程式碼再改
3. 如果涉及正式 UI / HUD / 玩家回饋，先確認 selected visual target、UI 視覺設計、動效方案與工具選型
4. 如果涉及 hit effect、技能、爆炸、粒子、shader、screen shake 或後製，先確認 Gameplay VFX 路由與架構邊界
5. 實作時維持 scope、non-goals、architecture boundary、visual target / style bible 與測試證據
6. 實作後驗證
7. 回報變更、風險、測試結果與 client decisions
...
3. 檢查 implementation 是否對齊 approved visual target 或 style bible
4. 檢查 reduced motion、reduced shake、效能、可讀性、overdraw 與輸入延遲
5. 檢查文件、里程碑 gate 與驗收標準
6. 回報最終產出、implementation delivery evidence 與 client acceptance 狀態
